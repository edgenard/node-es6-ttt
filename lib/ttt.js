...
const ask = (message, validInput, moveOn) => {
  rl.question(message, (response) => {
    if(validateInput(validInput, response)){
      rl.close()
      return moveOn(response)
    }else {
      ask(message, validInput, moveOn)
    }
  })
}

const chooseMark = (message, validInput, nextMove) => {
  rl.question(message, (response) => {
    if(validateInput(validInput, response)){
      rl.close()
      return nextMove({mark: response})
    }else{
      chooseMark(message, validInput, nextMove)
    }
  })
}

// const choosePosition = (message, currentBoard) => {
//   rl.question(message, (response) => {
...
// const nextMove = ({currentBoard=board(), mark, position}) => {
