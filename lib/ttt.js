...
export const gameState = ({gameType="1", gameBoard=board()}) => {
  return {
    gameType,
    gameBoard
  }
...
const chooseGameType = (message, validInput=["1", "2", "3"], nextStep=chooseMark) => {
  rl.question(message, (response) => {
    if(validateInput(validInput, response)){
      nextStep({nextMove: nextMove, gameType: response})
    } else {
      chooseGameType(gameMessages.invalidInput(validInput), validInput, nextStep)
    }
  })
