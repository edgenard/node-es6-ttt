  ...
  return {
    ...
    computerMove() {
      if(this.validMoves().length === 9){
        ...
        return this.placeMove(bestNextMove(this), state.turn)
      }
...
export const bestNextMove = (game) => {
  ...
  let winningMove = possibleMoves.find((game) => {
    return isWinningMove(game, game.turn())
  })

  if (winningMove){
    return winningMove.lastMove()
  }
  let losingMove = possibleMoves.find((game) => {
    return !(isLosingMove(game, game.turn()))
  })
  return losingMove.lastMove()
}

const isWinningMove = (game, playersMark) => {
  if(emptyPositions(game.getBoard()).length === 0) {
    return false
  }
  if (winner(game.getBoard())){
    return playersMark === game.turn()
  } else if(playersMark === game.turn()) {
    let nextMoves = possibleNextMoves(game.changeMarks())
    return nextMoves.some((game) => {
      return isWinningMove(game, playersMark)
    })
  }else {
    let nextMoves = possibleNextMoves(game.changeMarks())
    return nextMoves.every((game) => {
      return isWinningMove(game, playersMark)
    })
  }
}

const isLosingMove = (game, playersMark) => {
  if(emptyPositions(game.getBoard()).length === 0){
    return false
  }
  ...
  if(winner(game.getBoard())){
    ...
    let nextMoves = possibleNextMoves(game.changeMarks())
    return nextMoves.some((game) => {
      return isLosingMove(game, playersMark)
    })
  } else {
    let nextMoves = possibleNextMoves(game.changeMarks())
    return nextMoves.every((game) => {
...
const nextMove = (game, message=gameMessages.chooseSpot(game.validMoves())) => {
  ...
  if(game.humanPlayer()){
    ...
    nextMove(game.computerMove().changeMarks())
  }
