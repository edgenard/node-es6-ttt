...
export const gameStore = (state={
  ...
  return {
    setGameType(type) {
      switch (type) {
        case "1":
          state.player1.human = true
          state.player2.human = true
          break;
        case "2":
          state.player1.human = true
          state.player2.human = false
          break;
        default:
          state.player1.human = false
          state.player2.human = false
      }
    ...
    setMarks(mark) {
      state.player1.mark = mark
      state.player2.mark =  mark === "X" ? "O": "X"
      return gameStore(state)
    },
...
const chooseGameType = (message=gameMessages.gameType()) => {
  ...
  return rl.question(message, (response) => {
    if(validateInput(gameState.validGameTypes(), response)){
      rl.close()
      return chooseMark(gameStore(response))
    } else {
      return chooseGameType(gameMessages.invalidInput(gameState.validGameTypes()))
    }
  })

}

const chooseMark = (gameState, message=gameMessages.chooseMark()) => {
  let validMarks = gameState.validMarks()
  rl.question(message, (response) => {
    if(validateInput(validMarks, response)){

      return nextMove(gameState.setMarks(response))
    }else{
      return chooseMark(gameState, gameMessages.invalidInput(validMarks))
    }
  })
}
