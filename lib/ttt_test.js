...
describe("Tic Tac Toe", function () {
  ...
  describe("gameStore", function () {
    ...
    it("placeMove sets lastMove",function () {
      let game = gameStore().placeMove(5, "X")

      assert.equal(game.lastMove(), 5)
    })
  ...
  describe("Best Computer Move", function () {
    it("possibleNextMoves return array of all following states", function () {
      let game = gameStore().setGameType("3").setMarks("X").
                 placeMove(0, "X").placeMove(1, "O").placeMove(2, "X").
                 placeMove(3, "O").placeMove(4, "X").placeMove(5, "O")

      let followingStates = [
        game.placeMove(6, "X").getBoard(),
        game.placeMove(7, "X").getBoard(),
        game.placeMove(8, "X").getBoard()
      ]

      let nextBoards = possibleNextMoves(game).map((game) => game.getBoard())

      assert.deepEqual(nextBoards, followingStates )
    })

    it("bestNextMove returns immediate win", function () {
      let game = gameStore().setGameType("3").setMarks("X").
                 placeMove(0, "X").placeMove(1, "O").placeMove(2, "X").
                 placeMove(3, "O").placeMove(4, "X").placeMove(5, "O")

      assert.equal(bestNextMove(game), 6)
    })

    it("bestNextMove returns a winning move two moves away",function () {
      let game = gameStore().setGameType("3").setMarks("X").
                  placeMove(1, "O").placeMove(3, "O").placeMove(4, "X").
                  placeMove(5, "X").placeMove(7, "O")

      assert.equal(bestNextMove(game), 2)
    })

    it("bestNextMove returns immediate blocking move", function () {
      let game = gameStore().setGameType("3").setMarks("O").
                  placeMove(0, "O").placeMove(1,"O").
                  placeMove(2, "X").placeMove(4, "X")

      assert.equal(bestNextMove(game), 6)
    })

    it("bestNextMove return blocking move several moves away", function () {
      let game = gameStore().setGameType("3").setMarks("O").
                  placeMove(1, "O").placeMove(4, "X").placeMove(7, "X")

      assert.equal(bestNextMove(game), 0)
    })
  })
