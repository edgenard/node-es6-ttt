...
describe("Tic Tac Toe", function () {
  ...
  describe("gameStore", function () {
    ...
    it("placeMove returns new store with board", function () {
      ...
      gameState = gameState.placeMove(4, "X")

      assert.deepEqual(gameState.getBoard(),[[0, 1, 2], [3, "X", 5], [6, 7, 8]] )
    })
    ...
    it("validMoves returns empty spots on the board", function () {
      let gameState = gameStore()
      gameState = gameState.placeMove(8, "O")
      assert.deepEqual(gameState.validMoves(), [0, 1, 2, 3, 4, 5, 6, 7])
    })
    ...
    it("computerMove makes only move left", function () {
      let game = gameStore().placeMove(0, "X").placeMove(1, "O").placeMove(2, "X").placeMove(3, "O").placeMove(4, "X").placeMove(5, "O").placeMove(6, "O").placeMove(7, "X").setMarks("O").computerMove()
